...
def test_process_data_encodes_cat_and_creates_binary_labels(df):
    X, y, encoder, lb = process_data(
        df, categorical_features=["col2"], label="target", training=True, encoder=None, lb=None
    )
    X_expected = np.array([[1., 1., 0.], [2., 0., 1.]])
...
def test_process_data_needs_encoder_and_labelizer_when_training_is_false(df):
    with pytest.raises(AttributeError):
        process_data(
            df, categorical_features=["col2"], label="target", training=False, encoder=None, lb=None
        )
